	...
	<div class="outer">
		...
		<form action="<%=contextPath %>/insert.ph" method="post" id="enroll-form" enctype="multipart/form-data">
			...
			<table align="center">
				...
				<tr>
					<th>내용</th>
					<td colspan="3"><textarea rows="10" cols="30" name="content" style="resize:none;" required></textarea></td>
				</tr>
				<tr>
					<th>대표이미지</th>
					<td colspan="3" align="center"><img width="250" height="170" id="titleImg"></td>
				</tr>
				<tr>
					<th>상세이미지</th>
		...
		<script>
			...
			function loadImg(inputFile, num) {
				// inputFile: 현재 변화가 생긴 요소 객체, jquery에서 $(this)
				//console.log(inputFile.files);
				// inputFile.files: 업로드된 파일의 정보를 배열의 형태로 반환하는 속성
				// 파일 선택시 length는 1이 반환된다. 취소하면 파일정보가 없어지니 length가 0이 된다.
				if(inputFile.files.length == 1) {
					// 선택한 파일이 존재한다면
					// 해당 파일을 읽어서 영역에 썸네일 띄우기
					// 파일 읽어줄 객체 FileReader
					var reader = new FileReader();
					// 파일을 읽어줄 메소드: readAsDataURL(파일)
					// - 파일을 읽어들이는 순간 고유한 url을 부여한다.
					// 부여된 url을 src에 추가하면 해당 이미지를 띄워줄 수 있다.
					...
					reader.onload = function(e) { // e: 이벤트 객체
						...
						// 부여된 url은 target의 result 속성에 들어있다. 해당 result를 img src속성에 부여하면 미리보기 가능
						switch(num) {
						case 1:
