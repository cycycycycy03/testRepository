...
public class LoginController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 * - request: 서버로 요청할 때의 정보들이 담겨 있다. (요청시 전달값, 요청 전송 방식 등등)
		 * - response: 요청에 대한 응답에 필요한 객체
		 * 
		 * GET / POST 방식 차이
		 * - GET: 사용자가 입력한 값이 url에 노출되고 데이터의 길이 제한이 있으며 즐겨찾기를 할 수 있다.
		 * - POST: url에 노출되지 않고 데이터 길이 제한이 없으며 즐겨찾기를 할 수 없다.
		 * 
		 * */
		
		// 1) POST 요청일 경우 전달값에 한글이 포함되어 있다면 encoding설정을 해야 한다.
...
//		System.out.println("조회된 Member: " + m);
		...
		/*
		 * 응답페이지에 전달할 값이 있다면 값을 어딘가엔 담아서 전달해야 한다. (이 때 담아줄 수 있는 내장객체 4가지)
		 * - servlet scope
		 ...
		 * 3) request: request에 담은 데이터는 해당 request를 포워딩한 응답jsp에서만 꺼내쓸 수 있다.
		 * 4) page: 해당 jsp페이지에서만 꺼내쓸 수 있다.
		 * 
		 * 공통적으로 데이터를 담고자 한다면
		...
		if(loginUser == null) { // 로그인 실패 (에러 페이지로 응답해보기)
			...
			// 로그인한 회원의 정보를 계속 가지고 다닐 것이기 때문에 session에 담아준다.
			// session에 담으려면 Session 객체를 먼저 가지고 와야 한다.
			...
			// 응답 방식 2가지
			// 1. 위임(포워딩) - 요청을 유지한 채로 위임하는 방법 (url에 기존 주소가 남아있음)
			// RequestDispatcher view = request.getRequestDispatcher("응답페이지경로");
			// view.forward(request, response);
			// 2. 재요청(리다이렉트) - 새 페이지를 보여주는 방법 (url에 기존 주소가 남아있지 않음)
