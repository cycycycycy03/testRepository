...
public class BoardInsertController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * form에서 multipart/form-data 형식으로 전송했기 때문에 기존 request의 parameter영역에서 꺼낼 수 없다.
		...
		*/
		...
		// request를 multipart 객체로 이관시켜서 다뤄야 한다.
		...
		if(ServletFileUpload.isMultipartContent(request)) {
			 ...
			 * WebContent/resources/board_files 폴더 경로까지는 잡아주어야 한다. 해당 폴더에 저장될 것이기 때문에
			 * 
			 * */
			// 경로 마지막에 / 붙여주기 (그 폴더 안에 저장할 것이기 때문에)
			 ...
			 * 
			 * 위 구문 한 줄이 실행되는 순간 지정한 경로에 파일이 업로드된다.
			 * 사용자가 올린 파일명은 그대로 해당 폴더에 업로드하지 않는게 일반적이다. 같은 파일명이 있을 경우 덮어씌워질 경우도 있고
			 ...
			 * 기본적으로 제공된 객체는 파일명 수정을 파일명 뒤에 숫자를 카운팅하는 형식으로 진행해준다.
...
//			MultipartRequest multiRequest = new MultipartRequest(request, savePath, maxSize, "UTF-8", new DefaultFileRenamePolicy());
			...
			Attachment at = null; // 처음에는 null, 첨부파일이 있다면 객체 생성
			// multiRequest.getOriginalFileName("키");
			// 첨부파일이 있을 경우 원본명 반환, 없는 경우 null 반환
